...
const useDesignsFilter = (skeletonCount?: number | undefined) => {
  ...
  useEffect(() => {
    if(holder !== rnd){
        setRequest({
            data2: skeletonCount ? Array(skeletonCount).fill(0) : [],
            fetched: false,
            fetching: true,
            error: false,
          })
...
export default useDesignsFilter